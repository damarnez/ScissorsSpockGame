	...
	io.sockets.on('connection', function(socket) {
		...
		socket.on('items:create', function (data,callback) {
			...
			if(items.length === MAXUSERS){
				...
				var win_item_user = _.min(new_items,function(item){return item.item});
				var win_user = _.find(active_users,function(user){return user.id==win_item_user.id_user})
		...
		socket.on('NEW_GAME', function (data,callback) {
		   	console.log("Socket -> NEW_GAME");
		   	//callback(null, active_users);
